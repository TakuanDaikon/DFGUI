...
public class dfScrollPanel : dfControl
{
    ...
    [SerializeField] protected dfAtlas atlas;

    [SerializeField] protected string backgroundSprite;

    [SerializeField] protected Color32 backgroundColor = UnityEngine.Color.white;

    [SerializeField] protected bool autoReset = true;

    [SerializeField] protected bool autoLayout = false;

    [SerializeField] protected RectOffset scrollPadding = new RectOffset();

    [SerializeField] protected RectOffset flowPadding = new RectOffset();

    [SerializeField] protected LayoutDirection flowDirection = LayoutDirection.Horizontal;

    [SerializeField] protected bool wrapLayout = false;

    [SerializeField] protected Vector2 scrollPosition = Vector2.zero;

    [SerializeField] protected int scrollWheelAmount = 10;

    [SerializeField] protected dfScrollbar horzScroll;

    [SerializeField] protected dfScrollbar vertScroll;

    [SerializeField] protected dfControlOrientation wheelDirection = dfControlOrientation.Horizontal;

    [SerializeField] protected bool scrollWithArrowKeys = false;

    [SerializeField] protected bool useScrollMomentum = false;

    [SerializeField] protected bool useVirtualScrolling = false;

    [SerializeField] protected bool autoFitVirtualTiles = true;

    [SerializeField] protected dfControl virtualScrollingTile;
    ...
    public Vector2 ScrollPosition
    {
        ...
        set
        {
            ...
            var clientSize = new Vector2(this.size.x - this.scrollPadding.horizontal,
                this.size.y - this.scrollPadding.vertical);
    ...
    /// <param name="i"></param>
    /// <returns></returns>
    public dfPanel ScrollIntoIndex<T>(IList<T> backingList, int i)
    {
        Virtualize(backingList, i);


        List<IDFVirtualScrollingTile> tiels =
            (List<IDFVirtualScrollingTile>) virtualScrollData.GetType().GetField("Tiles").GetValue(virtualScrollData);
